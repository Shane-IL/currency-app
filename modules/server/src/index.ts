...
const updateCurrencyData = async () => {
    try {
        const url = `${currencyAPI}?apikey=${currencyAPIKey}&currencies=${JSON.stringify(
            currencies
        )}`;
...
const convertCurrency = async (
    ...
    return {
        exchange_rate: getExchangeRate(fromCurrencyRate, toCurrencyRate),
        currency_code: toCurrency,
        amount: toMaxDecimalPlaces(result, 0),
        error: "",
    };
};
...
app.get("/api/quote", async (req: Request, res: Response) => {
    ...
    const responseObject = await convertCurrency(
        fromCode,
        toCode,
        parseFloat(amount)
    );
...
app.listen(port, () => {
    console.log(
        `Currency converter server application is running on port ${port}.`
    );
});
